...
public class GenerateEnumEntries {
    ...
    public static void main(String[] args) throws URISyntaxException, IOException {
        String vcpJsonString = new String(Files.readAllBytes(Paths.get(GenerateEnumEntries.class.getClassLoader().getResource("vcp.json").toURI())));
        JSONArray vcpJson = new JSONArray(vcpJsonString);

        String enumString = IntStream.range(0, vcpJson.length())
                ...
                .map((j -> {
                    ...
                    IntStream.range(0, description.length())
                    ...
                    if (values != null) {
                        sb.append("new int[]{");
                        String s = IntStream.range(0, values.length())
                                .mapToObj(values::getString)
                                .map(GenerateEnumEntries::transformHexCode)
